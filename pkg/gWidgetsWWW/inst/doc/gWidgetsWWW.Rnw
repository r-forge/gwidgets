...
\item The \pkg{gWidgetsWWW} has a \texttt{RApache.conf} file that must
  be installed and configured. To install it with UBUNTU something
  like this will work (provided you have permission)
\begin{verbatim}
> system(sprintf("cp %s /etc/apache2", 
         system.file("templates/RApache.conf", package="gWidgetsWWW")))
> system("cat 'Include /etc/apache2 RApache.conf' >> /etc/apache2/apache2.conf")
\end{verbatim}
  
  To configure the \code{RApache.conf} file requires a few things:
 \begin{itemize}
